...
import com.illo.blm.config.KafkaProperties;
...
class BlmKafkaResourceIT {
    ...
    void setup() {
        ...
        BlmKafkaResource kafkaResource = new BlmKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/blm-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/blm-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
